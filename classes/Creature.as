package classes {
	...
	public class Creature extends VersionedSaveable implements IOwner {
		...
		*/
		...
		public function nippleFlagUnlocked(bRowIndex:int, newNippleFlag:Number):Boolean
		{
			if (hasLipples() || !nippleTypeUnlocked(0,0)) return false;
			return true;
		}
		
		public function nippleFlagLockedMessage():String
		{
		...
		*/
		...
		{
			if (breastRows[0].hasFlag(GLOBAL.NIPPLE_FLAG_HEART_AREOLA)) return true;
			else if (breastRows[0].hasFlag(GLOBAL.NIPPLE_FLAG_STAR_AREOLA)) return true;
			else return false;
		}
		public function areolaDescript():String
		{
			...
			//Flag descriptions
			if (breastRows[0].hasFlag(GLOBAL.NIPPLE_FLAG_PUFFY)) adjectives.push("puffy");
			if (breastRows[0].hasFlag(GLOBAL.NIPPLE_FLAG_HEART_AREOLA)) adjectives.push("heart-shaped");
			if (breastRows[0].hasFlag(GLOBAL.NIPPLE_FLAG_STAR_AREOLA)) adjectives.push("star-shaped");

			//If a player has a flag, they probably want to see stuff
			if (breastRows[0].nippleFlags.length > 0) adjectiveMin++;
		...
		public function nippleFlagDescript():String
		{
			var list:Array = new Array();
			var description:String = "";
			
			//Flag non-shape descriptions
			if (breastRows[0].hasFlag(GLOBAL.NIPPLE_FLAG_PUFFY)) list.push("puffy");
			
			//Default areola shape
			if (!hasSymbolAreola()) list.push("circular");
			
			//Others
			if (breastRows[0].hasFlag(GLOBAL.NIPPLE_FLAG_HEART_AREOLA)) list.push("heart-shaped");
			if (breastRows[0].hasFlag(GLOBAL.NIPPLE_FLAG_STAR_AREOLA)) list.push("star-shaped");
